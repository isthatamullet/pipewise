...
- **`pipewise inspect --keys`** — render inputs/outputs as a top-level structural summary (`'key': <type-summary>`) instead of value-truncated previews. Each entry shows the type and, for containers, the size: `'metadata': dict[7], 'extracted_items': list[15], 'body_text': str`. Designed for real-pipeline runs where steps share a lot of common content — the default value-truncated previews can look identical across steps when leading bytes match (e.g., enrichment pipelines that copy a base dict forward); `--keys` reveals what each step actually contributed. Mutually exclusive with `--full`; supplying both raises a usage error.
...
- **GitHub Action verdict line — three new cases for the `skipped` state**:
  - `⏭ All scorers skipped · no signal` — fires when every scorer in the report short-circuited (`passing == 0 AND failing == 0 AND skipped > 0`). Replaces the misleading "0/N passing" verdict that would otherwise show.
  - `⏭ {N} newly-skipped scorer · no regressions vs main` — fires when there are no regressions or active failures but PR-side scope narrowed (e.g., a scorer added `applies_to_step_ids` that excludes some steps). Doesn't trip CI red unless `pipewise diff --strict`.
  - `✅ All non-skipped scorers passing` — replaces "All scorers passing" copy when any PR-side scorer is skipped. "All passing" is a lie when some scorers didn't run.
- **No-baseline verdict line surfaces skipped count** — `🆕 N runs · P/T scorers passing (S skipped) · no baseline` when any scorer was skipped on PR side.
- **Per-row rollup table renders skipped as `⏭`** — the rollup table's pass/fail icon column now handles three states (`passed → ✅`, `failed → ❌`, `skipped → ⏭`). Was added in PR #1 alongside the schema flip; called out here for completeness.
- **`Plus:` extras footnote includes `newly skipped` and `newly running` counts** — surfaces tri-state transitions in the diff so the rollup-table row count reconciles with the main "Regressions / Improvements / Unchanged" row.
- **`ReportDiff.newly_skipped` / `ReportDiff.newly_running`** — two new buckets surfacing transitions that involve the `skipped` state. `newly_skipped` collects `passed → skipped` and `failed → skipped`; `newly_running` collects `skipped → passed` and `skipped → failed`. Visible in `pipewise diff` text + JSON output and the summary line.
- **`ReportDiff.has_strict_regressions()`** — companion to `has_regressions()` that returns True when there are regressions OR any `passed → skipped` transition. Used by `pipewise diff --strict`.
- **`pipewise-langgraph` reference adapter** ([`examples/langgraph/`](examples/langgraph/)) — in-tree subpackage adapting LangGraph's `create_react_agent` graph. Captures iterated nodes (`agent__N`, `tools__N`) via `graph.stream(stream_mode="updates")`, and explicit-skipped semantics (`status="skipped"`) for topology nodes that didn't fire during a run. Default scorers: `langgraph_messages_shape` (JSON-schema validation of the LangGraph messages-update shape) + `run_latency_30s`. Two committed golden runs exercising iteration and skipped-step semantics. Independent `pyproject.toml` pinning `langgraph>=1.0,<2.0`. Path-filtered `ci-langgraph.yml` workflow fires only on adapter-subdir changes. (#81)
- **`pipewise-anthropic-quickstarts` reference adapter** ([`examples/anthropic-quickstarts/`](examples/anthropic-quickstarts/)) — in-tree subpackage adapting an imperative-loop agent in the shape of the upstream [`agents`](https://github.com/anthropics/anthropic-quickstarts/tree/main/agents) Quickstart. Captures per-LLM-call (`agent__N`) and per-tool-call (`<tool_name>__N`) steps via an event-sink callback the agent loop fires inline. Per-step `cost_usd` computed from a small per-model price table; per-step `input_tokens`/`output_tokens` from `Message.usage`. Default scorers: `anthropic_agent_response_shape` (scoped to `agent__1..8` matching `DEFAULT_MAX_ITERATIONS`), `run_latency_60s`, `run_cost_10c` with `on_missing="skip"`. The bundled `MinimalAgent` mirrors upstream's loop shape; adopters with their own production agent code apply the same capture pattern directly. (#82)

### Changed

- **`ScoreResult` schema — tri-state `status` replaces `passed: bool` (BREAKING; pre-1.0)** — `passed: bool` removed; `status: Literal["passed", "failed", "skipped"]` added. `score: float | None` (was `float`) — a skipped scorer carries `score=None` because it didn't compute a score. New `model_validator` enforces `score is not None` when `status != "skipped"`. Two scenarios that previously returned `passed=True` without actually evaluating now return `status="skipped"`: step-id-scoping wrapper patterns where a scorer no-ops on out-of-scope steps, and budget scorers whose `on_missing="skip"` path fires with no upstream data. Old `report.json` files written before this change cannot be loaded — re-run `pipewise eval` to regenerate.
- **`CostBudgetScorer` / `LatencyBudgetScorer` `on_missing="skip"` path** — now emits `status="skipped"`, `score=None` (was `passed=True`, `score=1.0`). Indicates the scorer did not actually evaluate.
- **`applies_to_step_ids` kwarg on every built-in step scorer** — when set, scorers emit `status="skipped"` for out-of-scope step_ids without invoking their `score()` body. Replaces the prior wrapper-pattern workaround that returned false-greens. Defaults to `None` (run on every step, unchanged from prior behavior).
- **Runner auto-skips scorers on `step.status == "skipped"`** — when an upstream pipeline marks a step as skipped (e.g., a branching pipeline that didn't execute one branch, or a graph node that the conditional flow bypassed), the runner now emits a `skipped` `ScoreResult` directly without invoking the scorer. Failed steps still get scored — their partial outputs may carry signal.
...
- **`ScoreDiffEntry.passed_a` / `passed_b` removed; `status_a` / `status_b` added (BREAKING; pre-1.0)** — diff entries now carry the full tri-state instead of a derived boolean. The text-format `format_diff` output and JSON-format `--format json` output both reflect the rename. Adopters who consumed the JSON shape of `pipewise diff` directly will need to migrate.
- **`pipewise diff` `_placeholder_result()`** — entries absent from one side of the diff now carry `status="skipped"`, `score=None`, `reasoning="(absent)"` (was `status="failed"`). Reflects the truth that "absent" means "no result" rather than "failed."
...
- **`ci.yml` workflow** — switched from `paths-ignore` to "always-fire workflow, gate job body via `dorny/paths-filter`" pattern. Required status checks (`lint-typecheck-test (3.11)` + `(3.12)`) now report success on doc-only and adapter-subdir PRs instead of failing to fire, unblocking branch-protection rulesets that require those checks. Path filter is allowlist-shaped (matches `pipewise/**`, `tests/**`, `pyproject.toml`, `uv.lock`, and `ci.yml` itself) rather than the previous denylist. (#85)
- **Reference-adapter shape across public docs** — `docs/schema.md`, `docs/adapter-guide.md`, `docs/ci-integration.md`, `README.md`, `examples/README.md`, and `CLAUDE.md` rewritten to use `pipewise-langgraph` and `pipewise-anthropic-quickstarts` as the canonical worked examples. `docs/adapter-guide.md` adds a new "Capture vs adapter: run-time / eval-time split" architectural section teaching the run-time / eval-time pattern both new adapters use. The `examples/<framework>/` carve-out for in-tree reference adapters is reconciled across `CLAUDE.md`'s Architectural Non-Negotiables and `README.md`'s adapter-pattern diagram. (#83, #84, #86, #87, #88)
